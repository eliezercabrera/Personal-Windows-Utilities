...
{
    public static float GetChannelVolumeLevel(uint channelNumber)
    {
        VideoPlayerController.IAudioEndpointVolume masterVolume = null;
        try
        {
            masterVolume = VideoPlayerController.AudioManager.GetMasterVolumeObject();
            ...
            return masterVolume.GetChannelVolumeLevelScalar(channelNumber, out scalarLevel) == 0 ? 100*scalarLevel : -1.0f;
        }
        finally
        {
            if (masterVolume != null)
    ...
    public static void SetChannelVolumeLevel(uint channelNumber, float newLevel)
	{
		...
		try
		{
			...
			masterVolume.SetChannelVolumeLevelScalar(channelNumber, newLevel/100, Guid.Empty);
		}
		finally
		{
			if (masterVolume != null)
    ...
    public static void StepChannelVolumeLevel(uint channelNumber, float stepAmount)
    {
        float currentLevel = GetChannelVolumeLevel(channelNumber);
        Console.WriteLine(string.Format("currentLevel = {0}", currentLevel));
        if (currentLevel < 0.0f)
        {
            return;
        }

        float newLevel = currentLevel + stepAmount;
        Console.WriteLine(string.Format("newLevel before clamping = {0}", newLevel));

        newLevel = Math.Min(newLevel, 100.0f);
        newLevel = Math.Max(newLevel, 0.0f);

        Console.WriteLine(string.Format("newLevel after clamping = {0}", newLevel));
        SetChannelVolumeLevel(channelNumber, newLevel);
    }
}   